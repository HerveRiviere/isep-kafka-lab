...
\author{Hervé RIVIERE - herve.riviere@isep.fr}
\date{January 2020}
\maketitle
\section{Kafka use cases}

\begin{question}
  Indicate if Apache Kafka can fit \underline{or not} for the following use cases : 
\begin{enumerate}
\item Server log aggregation (being able to collect and read log or events from multiple servers and / or applications)
\item Transfert of files (from couple of kB to GB) between server and / or application 
\item Event bus allowing distinct applications to share and consume events (example : a web server is producing login events and we want the anti-fraud application and the CRM consuming and reacting to this event) 
...
\item Messaging system (like Whatsapp or Hangout)
\item Make data pipeline / streaming ETL (for instance transfert specific records from a database to elastic search and a file in FTP)
\end{enumerate}
...
Check ConsumerConfig.GROUP\textunderscore IDP\textunderscore CONFIG, compare it value with the value that was set 
...
Is the consumer application is at-least-once; at-most-once or exactly once ? 
How to have your consumer application exactly-once ? 
\end{question}
...
Kafka producers allows to have different mode of acknowledgement (ack setting). Open edu.isep.ii3502.kafka.ack.ProducerWithAck java class. 
...
\begin{question}
For all ack settings (0, 1, all) indicate if it's al-least-once (so generate duplicate); at-most-once (so can generate message lost) or exactly-once ? Why ? 
\end{question}
